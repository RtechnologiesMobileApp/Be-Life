...
class AuthRepoImpl implements AuthRepo{
  ...
  Future<String> login({required String email, required String pass})async {
    try{
      final  response= await apiService.postApi(url: ApiEndpoints.signIn,data: {"email":email,"password":pass});
  ...
  Future<String> register(UserModel userDetails) async{
    try{
   final  response= await apiService.postApi(url: ApiEndpoints.signup,data: userDetails.toJson());
   return response['token'];
    }catch(e){
     rethrow;
    }
